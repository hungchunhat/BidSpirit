    ...
    <section id="team" class="team">
        ...
        <div class="container d-flex justify-content-between align-items-center mb-4" data-aos="fade-up">
            <div class="btn-group" role="group" aria-label="Product Categories">
                <a href="/products"
                   class="btn form-btn{{ !request()->has('category_id') ? '-active' : '' }} category-btn">All</a>
                <a href="{{ request()->fullUrlWithQuery(['category_id' => '1', 'page' => null]) }}"
                   class="btn form-btn{{request('category_id')==1?'-active':''}} category-btn">Fine Art</a>
                <a href="{{ request()->fullUrlWithQuery(['category_id' => '2', 'page' => null]) }}"
                   class="btn form-btn{{request('category_id')==2?'-active':''}} category-btn">Antique</a>
                <a href="{{ request()->fullUrlWithQuery(['category_id' => '3', 'page' => null]) }}"
                   class="btn form-btn{{request('category_id')==3?'-active':''}} category-btn">Furniture</a>
                <a href="{{ request()->fullUrlWithQuery(['category_id' => '4', 'page' => null]) }}"
                   class="btn form-btn{{request('category_id')==4?'-active':''}} category-btn">Collectible</a>
                <a href="{{ request()->fullUrlWithQuery(['category_id' => '5', 'page' => null]) }}"
                   class="btn form-btn{{request('category_id')==5?'-active':''}} category-btn">Others</a>
            </div>
