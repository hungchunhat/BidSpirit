    ...
    <section id="team" class="team">
        ...
        <div class="container d-flex justify-content-between align-items-center mb-4" data-aos="fade-up">
            <div class="btn-group" role="group" aria-label="Product Categories">
                ...
                <a href="{{ request()->fullUrlWithQuery(['status' => 'live', 'page' => null]) }}" class="btn form-btn{{request('status')=='live'?'-active':''}} category-btn">Live</a>
                <a href="{{ request()->fullUrlWithQuery(['status' => 'upcoming', 'page' => null]) }}" class="btn form-btn{{request('status')=='upcoming'?'-active':''}} category-btn">Upcoming</a>
                <a href="{{ request()->fullUrlWithQuery(['status' => 'ended', 'page' => null]) }}" class="btn form-btn{{request('status')=='ended'?'-active':''}} category-btn">Completed</a>
            </div>
